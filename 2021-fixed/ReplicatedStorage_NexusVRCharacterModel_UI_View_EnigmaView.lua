...
-- mechanical fixes applied: %* -> %s x63, pairs() wrap x4, if-expr -> __2021_if x2, compound-assign x0
...
local function _(v15, v16, v17, v18) --[[ Line: 41 ]] --[[ Name: AddColor ]]
    return (("<font color=\"rgb(%s,%s,%s)\">%s</font>"):format(v16, v17, v18, v15));
end;
    ...
    if v11:GetSetting("Extra.EnigmaEnabled") == false then
        ...
        task.spawn(function() --[[ Line: 132 ]]
            ...
            while true do
                local v32 = true;
                local v33 = {};
                for v34, v35 in pairs(v31) --[[ 2021 ]] do
                    if not v35 then
                        if not v35 then
                            v32 = false;
                        end;
                ...
                for _, v37 in pairs(v33) --[[ 2021 ]] do
                    v31[v37] = true;
                end;
                if #v33 > 0 then
...
v14.UpdateText = function(v38) --[[ Line: 174 ]] --[[ Name: UpdateText ]]
    ...
    v39 = if v40 then if v10.Offsets.LeftFoot then ("%sLeft Foot Tracker: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">Active</font>"):format(0, 200, 0))) else ("%sLeft Foot Tracker: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">Requires Calibration</font>"):format(200, 150, 0))) else ("%sLeft Foot Tracker: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">Inactive</font>"):format(200, 0, 0))); -- << MANUAL FIX: if-expr (could not auto-convert) (2021 cannot compile this) >>
    v39 = if v41 then if v10.Offsets.RightFoot then ("%s\nRight Foot Tracker: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">Active</font>"):format(0, 200, 0))) else ("%s\nRight Foot Tracker: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">Requires Calibration</font>"):format(200, 150, 0))) else ("%s\nRight Foot Tracker: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">Inactive</font>"):format(200, 0, 0))); -- << MANUAL FIX: if-expr (could not auto-convert) (2021 cannot compile this) >>
    if l_Enigma_0:IsActive() then
        ...
        if not l_l_UserInputService_0_FocusedTextBox_0 or l_l_UserInputService_0_FocusedTextBox_0.Parent and l_l_UserInputService_0_FocusedTextBox_0.Parent.Name == "EnigmaTextBoxInput" then
            v39 = ("%s\nData transfer: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">Active</font>"):format(0, 200, 0)));
            if l_Enigma_0.Input then
                local v43 = string.split(l_Enigma_0.Input:GetCurrentText(), "|")[1];
                v39 = __2021_if(function() return v0[v43] end, function() return ("%s\n\nProtocol version: %s"):format(v39, v43) end, function() return ("%s\n\nProtocol version: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">%s</font>"):format(200, 150, 0, (("%s (Unsupported)"):format(v43))))) end);
            end;
        else
            v39 = ("%s\nData transfer: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">Inactive</font>"):format(200, 0, 0)));
        end;
        v39 = __2021_if(function() return not (not not v40 or v41) and l_Enigma_0:GetUserCFrameEnabled("None") end, function() return ("%s\n\n\226\154\160\239\184\143 Trackers are detected, but the role in SteamVR is \"None\". They need to have assigned roles through the SteamVR menu."):format(v39) end, function() return ("%s\n\nTo calibrate, stand up straight with your head level and facing forward with your feet next to each other pointing forward."):format(v39) end);
        ...
        v39 = ("%s\n\n\226\154\160\239\184\143 Enigma requires a desktop application. There may be experience-specific issues when Enigma is active."):format((("%s\nData transfer: %s"):format(v39, (("<font color=\"rgb(%s,%s,%s)\">Inactive</font>"):format(200, 150, 0)))));
        if v38.ShowTrackersButton then
...
v14.ShowDebugTrackers = function(v44) --[[ Line: 242 ]] --[[ Name: ShowDebugTrackers ]]
    ...
    if not l_Enigma_0.Enabled then
        ...
        while v44.DebugTrackersVisible do
            ...
            for _, v52 in pairs(l_Enigma_0.TrackerRoles) --[[ 2021 ]] do
                local v53 = 1;
                while l_Enigma_0:GetUserCFrameEnabled(v52, v53) do
                    v48[v53 == 1 and v52 or ("%s (%s)"):format(v52, v53)] = v50 * l_Enigma_0:GetUserCFrame(v52, v53);
                    v49 = v49 + 1;
                    v53 = v53 + 1;
                end;
            end;
            local v54 = 1;
            for v55, v56 in pairs(v48) --[[ 2021 ]] do
                if not v45[v54] then
