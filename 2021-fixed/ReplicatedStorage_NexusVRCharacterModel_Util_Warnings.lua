...
-- mechanical fixes applied: %* -> %s x1, pairs() wrap x1, if-expr -> __2021_if x0, compound-assign x0
...
return function() --[[ Line: 8 ]]
    ...
    if v2.Output and v2.Output.SuppressWarnings then
        for _, v5 in pairs(v2.Output.SuppressWarnings) --[[ 2021 ]] do
    ...
    if v3.all then
        ...
        for _, v10 in {
            ...
            if not v3[string.lower(v10.Key)] and v10.Condition() then
                warn(v10.Message);
                warn((("\tThis warning can be disabled by adding \"%s\" or \"All\" to Output.SuppressWarnings in the configuration of Nexus VR Character Model."):format(v10.Key)));
            end;
        end;
        return;
    end;
