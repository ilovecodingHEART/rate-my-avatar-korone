...
-- mechanical fixes applied: %* -> %s x0, pairs() wrap x0, if-expr -> __2021_if x1, compound-assign x2
...
function Util.MakeAliasCommand(name, commandString)
	...
	for arg in commandString:gmatch("$(%d+)") do
		if seenArgs[arg] == nil then
			...
			argType = __2021_if(function() return argType end, function() return argType:match("^%w+") end, function() return "string" end)
...
function Util.EmulateTabstops(text, tabWidth)
	...
	for i = 1, textLength do
		...
		if char == "\t" then
			...
			column = column + (spaces); --[[ 2021: was compound assignment ]]
		else
			table.insert(result, char)
			if char == "\n" then
				column = 0 -- Reset column counter on newlines
			elseif char ~= "\r" then
				column = column + (1); --[[ 2021: was compound assignment ]]
			end
		end
	end
