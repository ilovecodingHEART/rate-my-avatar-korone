...
-- mechanical fixes applied: %* -> %s x10, pairs() wrap x0, if-expr -> __2021_if x0, compound-assign x0
...
_replicatedstorge.Slerp.RemoteEvents.EquipJetpack.OnServerEvent:Connect(function(p1)
	...
	local _jetid = ("%s%s%s%s%s%s%s%s%s%s"):format(math.random(1, 9), math.random(1, 9), math.random(1, 9), math.random(1, 9), math.random(1, 9), math.random(1, 9), math.random(1, 9), math.random(1, 9), math.random(1, 9), math.random(1, 9));
