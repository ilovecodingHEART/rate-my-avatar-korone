...
-- mechanical fixes applied: %* -> %s x0, pairs() wrap x0, if-expr -> __2021_if x0, compound-assign x7
    ...
    describe("UpdateAsync", function()
        it("should update a key in the database", function()
            local Success, Data = PlayerData:UpdateAsync(
                "GetDataHere",
                function(old)
                    old.UpdateWhatever = old.UpdateWhatever .. (", world!"); --[[ 2021: was compound assignment ]]
                    return old
                end
            )
            ...
            local Callbacks = {
                ["Players"] = function(old)
                    for _, plr in pairs(old) do
                        plr.Level = plr.Level + (10); --[[ 2021: was compound assignment ]]
                        plr.Coins = plr.Coins + (100); --[[ 2021: was compound assignment ]]
                    end
                    return old
                end,
