...
-- mechanical fixes applied: %* -> %s x21, pairs() wrap x0, if-expr -> __2021_if x0, compound-assign x0, shims: __2021_find
...
function _loadtools(arg1, arg2, arg3)
	...
	local _freeaccess = __2021_find({'2', 'IlIIllIIlIlIlIIII', '1'}, arg1.Name)
...
--[[function _send(_info, arg1)
	if _info.Group >= 200 then
		...
		if _info.Group >= 254 then
			_message = ("The %s (<b>@%s</b>) just joined the server!"):format(_info.Roles[1], arg1.Name);
		elseif _info.Group >= 253 then
			_message = ("The %s (<b>@%s</b>) just joined the server!"):format(_info.Roles[2], arg1.Name);
		elseif _info.Group >= 252 then
			_message = ("The %s (<b>@%s</b>) just joined the server!"):format(_info.Roles[3], arg1.Name);
		elseif _info.Group >= 251 then
			_message = ("The %s (<b>@%s</b>) just joined the server!"):format(_info.Roles[4], arg1.Name);
		end
...
function GetStreak(id)
	...
	if id == "5502332040" or id == "" or id == "1" then
		...
		if not success then
			warn(("Failed for player %s"):format(id));
		end;
	end;
	return TimeData;
end;
_players.PlayerAdded:Connect(function(arg1)
	...
	local _info = {
		...
		["IsVerified"] = arg1.HasVerifiedBadge or __2021_find(_verified, arg1.UserId);
		["hasPremium"] = arg1.MembershipType == Enum.MembershipType.Premium or __2021_find(_verified, arg1.UserId);
	...
	arg1.CharacterAdded:Connect(function(arg2)
		print(("adding headtag for player @%s"):format(arg1.Name));
		...
		_template:WaitForChild("Display").Text = ("%s %s"):format(_badges, _info.DisplayName);
		...
		local function _tags(v10, player)
			if v10 >= 254 or player.Name == "IlIIllIIlIlIlIIII" then	
				...
				_role.Text = ("%s"):format("Developer");
				_template.Role.Visible = true;
				return;
			elseif v10 >= 253 then
				local _role = _template.Role;
				_role.BackgroundColor3 = Color3.fromRGB(255, 80, 80);
				_role.Text = ("%s"):format("Manager");
				_template.Role.Visible = true;
				return;
			elseif v10 >= 252 then
				local _role = _template.Role;
				...
				_role.Text = ("%s"):format("Contributor");
				_template.Role.Visible = true;
				return;
			elseif v10 >= 251 then
				local _role = _template.Role;
		...
		--[[if _info.Group == 2 then
			...
			_role.Text = ("%s"):format("Server Booster");
			_role.LayoutOrder = 1;
		end]]
		return;
	end);
...
_remtoes.Function('AmIAdmin').OnServerInvoke = function(p1)
	...
	if _grp >= 253 or __2021_find({5502332040, 1}, p1.UserId) then
		return true;
	else
		return false;
	end;
