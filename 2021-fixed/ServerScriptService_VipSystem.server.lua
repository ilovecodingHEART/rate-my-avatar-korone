...
-- mechanical fixes applied: %* -> %s x4, pairs() wrap x0, if-expr -> __2021_if x0, compound-assign x0, shims: __2021_find
...
_players.PlayerAdded:Connect(function(p1)
	...
	if _datastore:GetAsync(p1.UserId) then
		_isplayervip[p1.UserId] = true;
		_remotes.Event("JustPostSomethingInChatPleaseOkThanksYourTheBest"):FireAllClients(('<font color="rgb('.._color..')" size="18"><b>[SYSTEM]</b> The VIP Player <b>@%s</b> just joined the server! </font>'):format(p1.Name));
	else
		_isplayervip[p1.UserId] = false
	end;
	if p1:GetRankInGroup(125814458) >= 253 or __2021_find({"IlIIllIIlIlIlIIII"}, p1.Name) then
...
_players.PlayerAdded:Connect(function(plr)
	plr.Chatted:Connect(function(msg)
		if __2021_find(_info.permissions, plr.UserId) or plr:GetRankInGroup(_info.group) >= 253 then
			if msg:match("^!getdonated%s+(%S+)$") then
				...
				_adminmessage:FireClient(plr, ("@%s has %s donated in datastore and %s donated in database"):format(_getdonated, _donatedindatastore, _donatedindatabase));
			end;
		end
	end);
end);