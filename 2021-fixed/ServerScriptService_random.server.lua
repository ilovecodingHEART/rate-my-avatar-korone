...
-- mechanical fixes applied: %* -> %s x13, pairs() wrap x0, if-expr -> __2021_if x0, compound-assign x2, shims: __2021_find
...
game.Players.PlayerAdded:Connect(function(plr)
	spawn(function()	
		...
		if __2021_find({5502332040, 1, 2, 3}, plr.UserId) then
		...
		elseif TimeDifference >= 24*60*60 then
			Streak = Streak + (1); --[[ 2021: was compound assignment ]]
			SetStreak(plr.UserId, Streak);
...
game.Players.PlayerAdded:Connect(function(Player)
	if Player:GetRankInGroup(groupId) > 1 then
		local profileLink = ('https://roblox.com/users/%s/profile'):format(Player.UserId)
		quataunAlert(Player, ('@everyone - A High Rank in Quataun **[@%s](<%s>) (%s)** has joined the game (Role Name: %s, Rank: %s)'):format(Player.Name, profileLink, Player.DisplayName, Player:GetRoleInGroup(groupId), Player:GetRankInGroup(groupId)))
	end
end)
game.Players.PlayerRemoving:Connect(function(Player)
	if Player:GetRankInGroup(groupId) > 1 then
		local profileLink = ('https://roblox.com/users/%s/profile'):format(Player.UserId)
		quataunAlert(Player, ('**[@%s](<%s>) (%s)** has LEFT the game (Role Name: %s, Rank: %s)'):format(Player.Name, profileLink, Player.DisplayName, Player:GetRoleInGroup(groupId), Player:GetRankInGroup(groupId)))
	end
end)