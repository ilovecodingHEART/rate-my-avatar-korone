...
local addTriangleToFrame = function(paramScale : number)
	...
	if (axis < 3) then
		...
		size = (1/uiFrame.AbsoluteSize.X) * math.random(uiFrame.AbsoluteSize.X * 0.9, uiFrame.AbsoluteSize.X)
	elseif (axis >= 3) then
		y = axis == 3 and 0 or 1
		x = (1/uiFrame.AbsoluteSize.X) * math.random(0, uiFrame.AbsoluteSize.X)
		size = (1/uiFrame.AbsoluteSize.Y) * math.random(uiFrame.AbsoluteSize.Y * 0.9, uiFrame.AbsoluteSize.Y)
	end
	...
	uiTriangle.Size = UDim2.new(
		((math.random(1, 15)/100) * paramScale) * 1,
		0,
		(size * paramScale) * 0.7,
		0
	)
	...
	uiTriangle.Arrow.ImageColor3 = Color3.fromRGB(48, 0, 72)
