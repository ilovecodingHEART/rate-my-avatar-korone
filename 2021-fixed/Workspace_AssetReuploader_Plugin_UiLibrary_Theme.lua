...
-- WARNING: 3 Font.from*/Font.new/.FontFace line(s) remain (2022 Font value type); rewrite to Enum.Font by hand where it errors.
-- !! MANUAL FIX REQUIRED: 3 site(s) use if-expressions or `continue`,
...
local function normalizeRawTheme(rawTheme: RawTheme, themeId: string): Theme
	...
	for k, v in pairs(DEFAULT_THEME_PARAMETERS) --[[ 2021 ]] do
		if k == "Font" then
...
local function save()
	...
	local themesToSave = {}
	for themeId, theme in pairs(themes) --[[ 2021 ]] do
...
function Theme.init(currentPlugin: Plugin)
	...
	for themeId, theme in pairs(savedThemes) --[[ 2021 ]] do
