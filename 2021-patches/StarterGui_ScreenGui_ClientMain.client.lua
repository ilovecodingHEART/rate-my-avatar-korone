...
-- [2021] TextChatService does not exist on this client. This shim routes the game's
...
local function v120() --[[ Line: 262 ]] --[[ Name: updateUnclaimedDonationInbox ]]
	...
	local v110 = nil;
	v110 = __2021_if(function() return #v107 < v109 end, function() return string.format("Showing %s %i out of %i gifts", v105 == "Latest" and "latest" or "top donated", #v107, v109) end, function() return #v107 == 1 and "Showing 1 gift" or string.format("Showing %i gifts", #v107) end);
			...
			onClaimed = function() --[[ Line: 309 ]] --[[ Name: onClaimed ]]
				...
				if #v104 == 1 then
					...
					task.delay(0.5, function() --[[ Line: 318 ]]
						...
						for k in pairs(v104) do v104[k] = nil end; -- [2021] table.clear
					end);
...
task.spawn(function() --[[ Line: 359 ]]
	...
	while true do
		...
		v96 = v96 + 1;
		if v121 then
			local v122 = ("you received a \238\128\130%s gift from @%s!"):format(v121.robux, v121.username);
			...
			l_TextChatService_0.TextChannels.RBXGeneral:DisplaySystemMessage((("<font color= \"rgb(255, 30, 131)\">you received a \238\128\130%s gift from @%s!</font>"):format(v121.robux, v121.username)));
		else
...
local function v163() --[[ Line: 428 ]] --[[ Name: initsettings ]]
	...
	print((("Anonymous setting is %s"):format(v158.Anonymous == true)));
...
task.spawn(function()
	...
	for v170, v171 in pairs(v165) --[[ 2021 ]] do
...
local function v193(v176, v177, v178, v179)
	...
	local v181 = {};
	for _, v183 in pairs(v180) --[[ 2021 ]] do
	...
	for _, v187 in pairs(v180) --[[ 2021 ]] do
		...
		if v187.donatorUserId then
			...
			v188.Player1.Info.Username.Text = ("@%s"):format(v189.Username or "?");
			v188.Player1.Info.DisplayName.Text = v189.DisplayName or "?";
			v188.Player1.Avatar.Image = ("rbxthumb://type=AvatarHeadShot&id=%s&w=150&h=150"):format(v189.Id or 1);
		else
		...
		v188.Player2.Info.Username.Text = ("@%s"):format(v190.Username or "?");
		v188.Player2.Info.DisplayName.Text = v190.DisplayName or "?";
		v188.Player2.Avatar.Image = ("rbxthumb://type=AvatarHeadShot&id=%s&w=150&h=150"):format(v190.Id or 1);
		v188.Context.price.Amount.Text = ("%s"):format((v26.formatCommas(v187.donatedGiftbux)));
		v188.MouseButton2Click:Connect(function()
			if v187.donatorUserId then
				local v191 = v175[v187.donatorUserId] or {};
				l_ScreenGui_0.TransactionInfo2.Info.Donator.Value.Text = ("@%s"):format(v191.Username or "?");
			else
				l_ScreenGui_0.TransactionInfo2.Info.Donator.Value.Text = "Anonymous";
			end;
			local v192 = v175[v187.boothOwnerUserId] or {};
			l_ScreenGui_0.TransactionInfo2.Info.Reciever.Value.Text = ("@%s"):format(v192.Username) or "?";
			l_ScreenGui_0.TransactionInfo2.Info.Amount.price.Value.Text = ("%s"):format((v26.formatCommas(v187.donatedGiftbux)));
...
for _, v195 in ipairs(v164) do
	v195:GetPropertyChangedSignal("Text"):Connect(function()
		for _, v197 in pairs(v165) --[[ 2021 ]] do
...
l_SharingHistory_0.TopFrame.Buttons.Refresh.Activated:Connect(function()
	for _, v199 in pairs(v165) --[[ 2021 ]] do
...
task.spawn(function() --[[ Line: 554 ]]
	...
	for v171, v172 in pairs(v166) --[[ 2021 ]] do
...
local function v194(v177, v178, v179, v180) --[[ Line: 573 ]] --[[ Name: loadDonations ]]
	...
	local v182 = {};
	for _, v184 in pairs(v181) --[[ 2021 ]] do
	...
	for _, v188 in pairs(v181) --[[ 2021 ]] do
		...
		if v188.donatorUserId then
			...
			v189.Player1.Info.Username.Text = ("@%s"):format(v190.Username or "?");
			v189.Player1.Info.DisplayName.Text = v190.DisplayName or "?";
			v189.Player1.Avatar.Image = ("rbxthumb://type=AvatarHeadShot&id=%s&w=150&h=150"):format(v190.Id or 1);
		else
		...
		v189.Player2.Info.Username.Text = ("@%s"):format(v191.Username or "?");
		v189.Player2.Info.DisplayName.Text = v191.DisplayName or "?";
		v189.Player2.Avatar.Image = ("rbxthumb://type=AvatarHeadShot&id=%s&w=150&h=150"):format(v191.Id or 1);
		v189.Context.Amount.Text = ("\238\128\130%s"):format((v26.formatCommas(v188.donatedRobux)));
		...
		v189.MouseButton2Click:Connect(function() --[[ Line: 638 ]]
			if v188.donatorUserId then
				...
				l_ScreenGui_0.TransactionInfo.Info.Donator.Value.Text = ("@%s"):format(v192.Username or "?");
			else
				l_ScreenGui_0.TransactionInfo.Info.Donator.Value.Text = "Anonymous";
			end;
			local v193 = v176[v188.boothOwnerUserId] or {};
			l_ScreenGui_0.TransactionInfo.Info.Reciever.Value.Text = ("@%s"):format(v193.Username) or "?";
			l_ScreenGui_0.TransactionInfo.Info.Amount.Value.Text = ("\238\128\130%s"):format((v26.formatCommas(v188.donatedRobux)));
...
for _, v196 in ipairs(v165) do
	v196:GetPropertyChangedSignal("Text"):Connect(function() --[[ Line: 670 ]]
		for _, v198 in pairs(v166) --[[ 2021 ]] do
...
l_DonationHistory_0.TopFrame.Buttons.Refresh.Activated:Connect(function() --[[ Line: 688 ]]
	for _, v200 in pairs(v166) --[[ 2021 ]] do
...
local function v221() --[[ Line: 711 ]] --[[ Name: loadServers ]]
	...
	for _, v206 in pairs(v202) --[[ 2021 ]] do
		local v207 = nil;
		v207 = __2021_if(function() return v206.isDeluxe end, function() return l_UITemplates_0.DeluxeServerFrame:Clone() end, function() return l_UITemplates_0.ServerFrame:Clone() end);
		v207.Info.Title.Text = v206.serverName;
		v207.Info.Region.Text = v206.language:upper();
		v207.Avatars.PlayerCount.Text = ("%s/%s"):format(v206.playerCount, v206.maxPlayers);
		v207.LayoutOrder = -v206.playerCount;
		v207.Parent = l_ServerSelector_0.Frame;
		for v208, v209 in pairs(v206.userIds) --[[ 2021 ]] do
			if v208 < 10 then
				local v210 = l_UITemplates_0.Avatar:Clone();
				v210.Image = ("rbxthumb://type=AvatarHeadShot&id=%s&w=150&h=150"):format(v209);
				v210.ZIndex = v208;
...
v24.Compat(v258):onChange(function() --[[ Line: 882 ]]
	...
	task.spawn(function() --[[ Line: 953 ]]
		...
		elseif v269 then
			...
			script.Activity.Value = ("Hovering on %s Robux button"):format(l_v269_Attribute_2);
			...
			if l_LocalPlayer_0.UserId ~= l_v269_Attribute_5 then
				v268 = v23.delay(0.5):andThen(function() --[[ Line: 970 ]]
					...
					if __2021_if(function() return l_v269_Attribute_1 == "Gamepass" end, function() return l_MarketplaceService_0:UserOwnsGamePassAsync(l_LocalPlayer_0.UserId, l_l_v269_Attribute_0_0) end, function() return l_MarketplaceService_0:PlayerOwnsAsset(l_LocalPlayer_0, l_l_v269_Attribute_0_0) end) then
			...
			l_ScreenGui_0.ButtonInfo.BackgroundTransparency --[[ 2021: was GroupTransparency (CanvasGroup, 2022) ]] = 1;
...
l_UserInputService_0.InputBegan:Connect(function(v288, v289) --[[ Line: 1020 ]]
	local v290 = __2021_if(function() return v266 and v266:IsDescendantOf(game) end, function() return v266 end, function() return nil end);
	if v290 then
		if not v289 and v288.KeyCode == Enum.KeyCode.G and l_UserInputService_0:IsKeyDown(Enum.KeyCode.LeftShift) then
			...
			if l_v290_Attribute_0 == "Gamepass" then
				local v293 = ("https://www.roblox.com/game-pass/%s/"):format(l_v290_Attribute_1);
				...
				local v294 = ("https://www.roblox.com/catalog/%s/"):format(l_v290_Attribute_1);
				l_ScreenGui_0.Prompt.TopFrame.Title.Text = "CONTINUE";
...
l_UserInputService_0.InputEnded:Connect(function(v298, _) --[[ Line: 1038 ]]
	local v300 = __2021_if(function() return v266 and v266:IsDescendantOf(game) end, function() return v266 end, function() return nil end);
...
local function v343() --[[ Line: 1100 ]] --[[ Name: applyCustomization ]]
	...
	l_ScreenGui_0.Debug.Text = ("%s \n %s \n %s \n %s \n %s \n %s"):format(l_Attribute_1, v321, l_Attribute_2, l_Attribute_3, l_Attribute_4, l_Attribute_5);
...
l_ScreenGui_0.EditBoothText.Frame.TextBox.FocusLost:Connect(function() --[[ Line: 1180 ]]
	...
	if v373 < string.len(l_Text_2) then
		...
		local v374 = ("exceeded character limit %s"):format(v373);
...
l_ScreenGui_0.EditBoomboxId.Frame.TextBox.FocusLost:Connect(function() --[[ Line: 1228 ]]
	...
	local v410 = ("set sound id to %s!"):format(l_Text_4);
	...
	local function v648(v500, v501) --[[ Line: 1487 ]] --[[ Name: addNewBoothButton ]]
		...
		local function v609() --[[ Line: 1497 ]] --[[ Name: onActivated ]]
			...
			local v598 = v29(function() --[[ Line: 1549 ]]
				...
				if v501.icon.Image == nil then
					...
					warn((("Booth icon for %s is nil"):format(v501.name)));
					return;
				else
			...
			if v598 then
				...
				error((("Error occured while setting booth model: %s"):format(v598)));
			end;
		end;
		v503 = v33({
			text = v500, 
		...
		v610.Activated:Connect(function() --[[ Line: 1590 ]]
			...
			elseif v475:get() ~= v500 then
				...
				for v636, v637 in pairs(v631) --[[ 2021 ]] do
					...
					do
						...
						pcall(function() --[[ Line: 1627 ]]
							if v473[v639] then
								...
								l_l_default_0_0 = color3_fromHex_2021(v473[v639]);
							end;
						end);
...
v30.OnClientEvent("PurchasePopup"):Connect(function(v701) --[[ Line: 2089 ]]
	v78(false);
	local v702 = ("purchased %s!"):format(v701);
...
v30.OnClientEvent("RedeemCode"):Connect(function(v722, v723) --[[ Line: 2106 ]]
	...
	if v722 == "success" then
		l_SoundService_0.SFX.Success:Play();
		l_SoundService_0.SFX.ItemPurchase:Play();
		local v724 = ("redeemed %s!"):format(v723);
...
v30.OnClientEvent("ChatDonationAlert"):Connect(function(v794, v795, v796, v797) --[[ Line: 2135 ]]
	...
	if v797 == "admin" then
		l_TextChatService_0.TextChannels.RBXGeneral:DisplaySystemMessage((("<font color= \"rgb(255, 100, 72)\">%s</font"):format(v796)));
		return;
	else
		...
		l_TextChatService_0.TextChannels.RBXGeneral:DisplaySystemMessage(v798 .. string.format(("<font size=\"18\">%s</font>"):format(v800), v794, v801, v795));
		return;
	end;
end);
v30.OnClientEvent("ChatAlert"):Connect(function(v916, v917, v918) --[[ Line: 2482 ]]
	if v917 == "announcement" then
		...
		l_TextChatService_0.TextChannels.RBXGeneral:DisplaySystemMessage((("<font size=\"21\">%s</font>"):format(v916)));
		return;
	elseif v917 == "giftpass" then
		...
		l_TextChatService_0.TextChannels.RBXGeneral:DisplaySystemMessage((("<font size=\"18\" color= \"rgb(0, 255, 234)\">%s</font>"):format(v916)));
		return;
	else
		l_TextChatService_0.TextChannels.RBXGeneral:DisplaySystemMessage((("<font size=\"18\" color= \"rgb(0, 255, 234)\">%s</font>"):format(v916)));
		return;
