...
return table.freeze({
	["target"] = function(p199, p200, p201, p202)
		...
		if not ("Instance"):find((typeof(p199))) then
			error(("bad argument #%* to %* (%* expected, got %*)"):format(1, "spr.target", "Instance", (typeof(p199))), 3)
		end
		if not ("number"):find((typeof(p200))) then
			error(("bad argument #%* to %* (%* expected, got %*)"):format(2, "spr.target", "number", (typeof(p200))), 3)
		end
		if not ("number"):find((typeof(p201))) then
			error(("bad argument #%* to %* (%* expected, got %*)"):format(3, "spr.target", "number", (typeof(p201))), 3)
		end
		if not ("table"):find((typeof(p202))) then
			error(("bad argument #%* to %* (%* expected, got %*)"):format(4, "spr.target", "table", (typeof(p202))), 3)
		end
	...
	["stop"] = function(p210, p211)
		...
		if not ("Instance"):find((typeof(p210))) then
			error(("bad argument #%* to %* (%* expected, got %*)"):format(1, "spr.stop", "Instance", (typeof(p210))), 3)
		end
		if not ("string|nil"):find((typeof(p211))) then
			error(("bad argument #%* to %* (%* expected, got %*)"):format(2, "spr.stop", "string|nil", (typeof(p211))), 3)
		end
		if p211 then
	...
	["completed"] = function(p213, p214)
		...
		if not ("Instance"):find((typeof(p213))) then
			error(("bad argument #%* to %* (%* expected, got %*)"):format(1, "spr.completed", "Instance", (typeof(p213))), 3)
		end
		if not ("function"):find((typeof(p214))) then
			error(("bad argument #%* to %* (%* expected, got %*)"):format(2, "spr.completed", "function", (typeof(p214))), 3)
		end
