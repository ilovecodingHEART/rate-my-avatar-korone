	...
	if not v37.initialized then
		...
		if not l_driver_0 then
			...
			if not l_Character_0 then
				...
				for _, v44 in l_engineSounds_0 do
					v44.sound.Looped = true;
					v44.sound:Play();
					v44.sound.PlaybackSpeed = v44.minPlaybackSpeed;
				end;
				for _, v46 in l_driftSounds_0 do
					v46.sound.Looped = true;
					v46.sound.Volume = 0;
					v46.sound:Play();
				end;
				for _, v48 in l_exhaustParticles_0 do
...
local _ = function(v50) --[[ Line: 133 ]] --[[ Name: sharedOnCarExited ]]
	...
	for _, v55 in l_engineSounds_1 do
		v55.sound:Stop();
	end;
	for _, v57 in l_driftSounds_1 do
		v57.sound:Stop();
	end;
	for _, v59 in l_exhaustParticles_1 do
...
local _ = function(v72, v73) --[[ Line: 169 ]] --[[ Name: setBrakeLightsAreEnabled ]]
	...
	if not v72.initialized then
		...
		local _ = v72.shared;
		for _, v76 in v72.shared.reverseLightParts do
			if v73 then
...
v9.onCarEntered = function(v78) --[[ Line: 181 ]] --[[ Name: onCarEntered ]]
	...
	if not v78.initialized then
		...
		if v78.initialized then
			...
			local _ = v78.shared;
			for _, v85 in v78.shared.reverseLightParts do
...
v9.onCarExited = function(v87) --[[ Line: 212 ]] --[[ Name: onCarExited ]]
	...
	if not v87.initialized then
		...
		if v87.initialized then
			...
			local _ = v87.shared;
			for _, v90 in v87.shared.reverseLightParts do
...
local _ = function(v176) --[[ Line: 379 ]] --[[ Name: getEngineThrust ]]
	...
	if v181 then
		...
		local v184 = nil;
		for _, v186 in l_reverseEngineThrustCurve_0 do
			local v187 = v186[1];
			local v188 = v186[2];
			if v187 == v182 then
				return v188;
		...
		local v190 = nil;
		for _, v192 in l_forwardEngineThrustCurve_0 do
			local v193 = v192[1];
			local v194 = v192[2];
			if v193 == v182 then
				return v194;
...
v9.setLightsAreEnabled = function(v196, v197) --[[ Line: 396 ]] --[[ Name: setLightsAreEnabled ]]
	...
	for _, v202 in l_frontLightParts_0 do
		if v197 then
	...
	for _, v204 in l_tailLightParts_0 do
		if v197 then
...
local _ = function(v205, v206) --[[ Line: 407 ]] --[[ Name: setReverseLightsAreEnabled ]]
	-- upvalues: v68 (copy)
	for _, v208 in v205.shared.reverseLightParts do
		if v206 then
...
local function v232(_, v211) --[[ Line: 415 ]] --[[ Name: preSimulationShared ]]
	...
	if not v211.initialized then
		...
		for _, v227 in v211.shared.engineSounds do
		...
		for _, v230 in v211.shared.exhaustParticles do
	...
	if not v241.initialized then
		...
		if l_initialized_0.throttle ~= 0 and v253 ~= -math.sign(l_initialized_0.throttle) then
			...
			if v260 then
				...
				local v264 = nil;
				for _, v266 in l_reverseEngineThrustCurve_1 do
					local v267 = v266[1];
					local v268 = v266[2];
					if v267 == v261 then
						v262 = v268;
				...
				local v270 = nil;
				for _, v272 in l_forwardEngineThrustCurve_1 do
					local v273 = v272[1];
					local v274 = v272[2];
					if v273 == v261 then
						v262 = v274;
		...
		for v279, v280 in l_shared_7.wheelRecords do
		...
		if v295 and l_initialized_0.brakeLightsAreEnabled == false then
			if v241.initialized then
				...
				local _ = v241.shared;
				for _, v298 in v241.shared.reverseLightParts do
		...
		elseif not v295 and l_initialized_0.brakeLightsAreEnabled == true and v241.initialized then
			...
			local _ = v241.shared;
			for _, v301 in v241.shared.reverseLightParts do
...
v9.bystanderPreRender = function(v308) --[[ Line: 601 ]] --[[ Name: bystanderPreRender ]]
	...
	if not l_initialized_1 then
		...
		for v316, v317 in l_shared_11.wheelRecords do
...
v9.bystanderPreSimulation = function(v333, v334) --[[ Line: 656 ]] --[[ Name: bystanderPreSimulation ]]
	...
	if not l_initialized_2 then
		...
		for v349, v350 in l_shared_12.wheelRecords do
			...
			local v357 = nil;
			v357 = if v356 then v356.Distance else l_suspensionSpringTargetHeight_1;
		...
		if v364 then
			if l_initialized_2.brakeLightsAreEnabled == false and v334.initialized then
				...
				local _ = v334.shared;
				for _, v367 in v334.shared.reverseLightParts do
		...
		elseif l_initialized_2.brakeLightsAreEnabled == true and v334.initialized then
			...
			local _ = v334.shared;
			for _, v370 in v334.shared.reverseLightParts do
...
v9.throttleSignChanged = function(v371) --[[ Line: 765 ]] --[[ Name: throttleSignChanged ]]
	...
	if not l_initialized_3 then
		...
		if not v371.initialized then
			...
			local _ = v371.shared;
			for _, v376 in v371.shared.reverseLightParts do
				if v373 then
