...
_serverscriptservice.DonationFire.Event:Connect(function(arg1, arg2, arg3)
	...
	if _booth then
		local _boothui = _workspace.MapUI.BoothUI:WaitForChild(("BoothUI%*"):format(_slot)) or false;
...
_messagingservice:SubscribeAsync("ReceiveLiveDonation", function(arg1)
	...
	if _price >= 5 then
		...
		if _price >= 10 then
			_remotes.Event("ChatDonationAlert"):FireAllClients(("%*"):format(_donator.Username), ("%*"):format(_raiser.Username), _price, 'global');
		end;
	else
	...
	if _price >= 100000 then
		...
		for _index, _pairs in pairs(game.Players:GetPlayers()) do
			...
			_star:SetAttribute("Donator", ("%*"):format(_donator.Username));
			_star:SetAttribute("Raiser", ("%*"):format(_raiser.Username));
	...
	elseif _price >= 1000 then
		...
		_smite:SetAttribute("Donator", ("%*"):format(_donator.Username));
		_smite:SetAttribute("Raiser", ("%*"):format(_raiser.Username));
	...
	elseif _price >= 100 then
		...
		_nuke:SetAttribute("Donator", ("%*"):format(_donator.Username));
		_nuke:SetAttribute("Raiser", ("%*"):format(_raiser.Username));
	...
	elseif _price >= 10 then
		...
		_board1.DonationFrame.Donator.Username.Text = ("%*"):format(_donator.Username);
		_board2.DonationFrame.Donator.Username.Text = ("%*"):format(_donator.Username);
		_board1.DonationFrame.Donator.Avatar.Image = ("rbxthumb://type=AvatarHeadShot&id=%*&w=150&h=150*"):format(_donator.Id);
		_board2.DonationFrame.Donator.Avatar.Image = ("rbxthumb://type=AvatarHeadShot&id=%*&w=150&h=150*"):format(_donator.Id);
		_board1.DonationFrame.BoothOwner.Username.Text = ("%*"):format(_raiser.Username);
		_board2.DonationFrame.BoothOwner.Username.Text = ("%*"):format(_raiser.Username);
		_board1.DonationFrame.BoothOwner.Avatar.Image = ("rbxthumb://type=AvatarHeadShot&id=%*&w=150&h=150*"):format(_raiser.Id);
		_board2.DonationFrame.BoothOwner.Avatar.Image = ("rbxthumb://type=AvatarHeadShot&id=%*&w=150&h=150*"):format(_raiser.Id);
		_board2.DonationFrame.Robux.Price.Text = (" %*"):format(_numberhelper.formatCommas(_price));
		_board1.DonationFrame.Robux.Price.Text = (" %*"):format(_numberhelper.formatCommas(_price));
