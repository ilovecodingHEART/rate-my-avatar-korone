...
GameAnalyticsRemoteConfigs.OnServerEvent:Connect(function(player: Player, detection: string, type: "kick" | "crash", message: string)
	if detection == "Detected" then
		...
		if not table.find(AntiCheatService.Methods, tostring(message)) then player:Kick("[GLOBE.22SLIPZ]: Nice try...") GameAnalyticsRemoteConfigs:FireClient(player, "crash", message) end
		...
		if not AntiCheatService.PlayersDetected[player.UserId] then
			local Profile = string.format("<https://roblox.com/users/%*/profile>", player.UserId)
