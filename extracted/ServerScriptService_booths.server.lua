...
function _boothname(arg1)
	...
	for i = 1, #arg1 do
		...
		if not table.find(_letters, _letter) or i == 1 then
			_name = _name .. _letter;
		else
			_name = ("%* %*"):format(_name, _letter);
		end;
	end;
	return _name;
end;
...
function _getdisplayname(arg1)
	...
	local _display = ("%*'s Stand"):format(arg1.DisplayName);
...
function _changebooth(arg1, arg2, arg3)
	...
	local _boothui = _workspace.MapUI.BoothUI:WaitForChild(("BoothUI%*"):format(arg3));
	if not _booth then
...
_remotes.Function("RefreshItems").OnServerInvoke = function(p1)
	...
	local _boothui = _workspace.MapUI.BoothUI:WaitForChild(("BoothUI%*"):format(_boothslotsave[p1.UserId]));
	...
	if _donatesign(p1) then
		--[[if game:GetService("MarketplaceService"):UserOwnsGamePassAsync(p1.UserId, 1679943835) then
			if _hugedonatesign(p1) then
				_hugedonatesign(p1).TextSign.SurfaceGui.TextLabel.Text =  ("%*"):format(_text);
			end;
		end;]]
		_donatesign(p1).TextSign.SurfaceGui.TextLabel.Text =  ("%*"):format(_text);
		spawn(function()
...
_remotes.Event("UnclaimBooth").OnServerEvent:Connect(function(p1)
	...
	_boothui.Details.Owner.Text = ("%*"):format("unclaimed");
	_boothui.Sign.TextLabel.Text = ("%*"):format("your text here");
...
_remotes.Function("ClaimBooth").OnServerInvoke = function(arg1, arg2)
	...
	local _boothui = _workspace.MapUI.BoothUI:WaitForChild(("BoothUI%*"):format(arg2));
	_boothui.Details.Owner.Text = _getdisplayname(arg1);
	_boothui.Details.Raised.Visible = true;
	_boothui.Details.Raised.Text = ("%* raised"):format(_numberhelper.formatCommas(arg1:GetAttribute("RaisedNumber")));
	spawn(function()
			...
			for _, part in pairs(booth:GetDescendants()) do
				if part:IsA("BasePart") and part:GetAttribute("ColorId") then
					...
					if savedColors[colorId] then
						part.Color = Color3.fromHex(savedColors[colorId])
					end
				end
			end
		else
...
game.Players.PlayerAdded:Connect(function(arg1)
	arg1.CharacterAdded:Connect(function(plr)
		...
		if _again == true then
			...
			_donatesign(arg1).TextSign.SurfaceGui.TextLabel.Text =  ("%*"):format(_setsigntext:GetAsync(arg1.UserId) or "your text here");
			--[[if game:GetService("MarketplaceService"):UserOwnsGamePassAsync(arg1.UserId, 1679943835) then
				_items:LoadPlayerItemsOnBooth(arg1, _hugedonatesign(arg1).DonateSign.SignGui.Frame , _customizationdata(arg1), false);
				_hugedonatesign(arg1).TextSign.SurfaceGui.TextLabel.Text =  ("%*"):format(_setsigntext:GetAsync(arg1.UserId) or "your text here");
